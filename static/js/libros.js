    ...
    try {
        ...
        tbody.innerHTML = data.libros.map(l => `
            <tr>
                ...
                <td>
                    <button class="btn-edit" onclick="editarLibro(${l.id}, '${l.titulo}', ${l.cantidad})">✏️ Editar</button>
                    <button class="btn-danger" onclick="eliminarLibro(${l.id})">🗑️ Borrar</button>
                </td>
            </tr>
        `).join('');
...
async function editarLibro(id, titulo, cantidadActual) {
    const nuevaCantidad = prompt(`Editar cantidad para "${titulo}":`, cantidadActual);
