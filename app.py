...
def metricas():
    try:
        ...
        # Préstamos vencidos (+7 días)
        try:
            c.execute("SELECT COUNT(*) FROM reservas WHERE estado='prestado' AND fecha_prestamo IS NOT NULL AND fecha_prestamo < NOW() - INTERVAL '7 days'")
        ...
        try:
            c.execute("""
                ...
                LIMIT 5
            """)
            cats = [{"categoria": row[0], "total": row[1]} for row in c.fetchall()]
        except Exception:
            cats = []

        # Alumnos activos (con reservas)
        try:
            c.execute("""
                SELECT u.nombre, u.email, u.username, COUNT(r.id) as total
                FROM usuarios u
                JOIN reservas r ON r.usuario_id = u.id
                WHERE u.rol = 'alumno'
                GROUP BY u.id, u.nombre, u.email, u.username
                ORDER BY total DESC
                LIMIT 5
            """)
            alumnos_activos = [{"nombre": row[0], "email": row[1], "username": row[2], "total": row[3]} for row in c.fetchall()]
        except Exception:
            alumnos_activos = []
        ...
        return jsonify({
            ...
            "categorias_pedidas": cats,
            "alumnos_activos":    alumnos_activos,
        })
    except Exception:
        traceback.print_exc()
        return jsonify({"error": "Error al cargar métricas"}), 500
